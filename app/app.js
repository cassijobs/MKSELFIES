...
function home(){stopCamera();const pending=sessionStorage.getItem('mkselfies_camera_pending')==='1';app.innerHTML=`<section class="screen"><span class="brand">MKSELFIES</span><h1>A arte guarda uma surpresa.</h1><p>Aponte a câmera para o presente. Quando ele for reconhecido, uma experiência fotográfica será revelada.</p><div class="art-orbit"><span class="spark s1">✦</span><span class="mug">☕</span><span class="spark s2">✦</span></div><input id="recover-photo" type="file" accept="image/*" hidden><div class="actions">${pending?'<button class="btn primary" id="recover">Usar a foto que acabei de tirar</button>':''}<button class="btn ${pending?'secondary':'primary'}" id="scan">Experimentar protótipo</button><button class="btn quiet" id="studio">Abrir Studio</button></div></section>`;document.querySelector('#scan').onclick=reveal;document.querySelector('#studio').onclick=()=>location.href='../studio/';const recover=document.querySelector('#recover');if(recover){const input=document.querySelector('#recover-photo');recover.onclick=()=>input.click();input.onchange=()=>{const file=input.files?.[0];if(file){sessionStorage.removeItem('mkselfies_camera_pending');openPhotoEditor(file)}}}}
...
function openCamera(){
 stopCamera();
 app.innerHTML=`<section class="screen native-prompt"><span class="brand">Sua foto encantada</span><h1>Abra a câmera do celular.</h1><p>Tire sua fotografia normalmente, em pé. Ao voltar, você poderá posicionar o efeito antes de guardar.</p><div class="art-orbit"><span class="spark s1">✦</span><span class="mug">📷</span><span class="spark s2">✦</span></div><input id="native-camera" type="file" accept="image/*" capture="user" hidden><input id="gallery-photo" type="file" accept="image/*" hidden><div class="actions"><button class="btn primary" id="open-native">Abrir câmera frontal</button><button class="btn secondary" id="open-gallery">Escolher uma foto</button><button class="btn quiet" id="back">Voltar</button></div></section>`;
 const input=document.querySelector('#native-camera'),gallery=document.querySelector('#gallery-photo');
 const receive=file=>{if(file){sessionStorage.removeItem('mkselfies_camera_pending');openPhotoEditor(file)}};
 input.onchange=()=>receive(input.files?.[0]);gallery.onchange=()=>receive(gallery.files?.[0]);
 document.querySelector('#open-native').onclick=()=>{sessionStorage.setItem('mkselfies_camera_pending','1');input.click()};
 document.querySelector('#open-gallery').onclick=()=>gallery.click();
 document.querySelector('#back').onclick=home;
}
